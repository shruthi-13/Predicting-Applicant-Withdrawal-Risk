...
tfa_data$`Essays Sentiment` <- factor(tfa_data$`Essays Sentiment`, levels = c(0,1), labels = c("Positive","Negative"))
...
factor_cols <- c("App Started Year (RTAT)", "Is Math, Sci, or Eng Major Minor", "Major 1", "Major 2",
                 ...
                 "Attended Event", "Essays Sentiment", "submitted_early", "quick_starter", "quick_completer")
...
data_numeric <- dummy_cols(tfa_data, select_columns = factor_cols, remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
# Train-Test Split
train_idx_tree <- createDataPartition(data_tree$`Completed Admissions Process`, p = 0.8, list = FALSE)
train_tree <- data_tree[train_idx_tree, ]
test_tree <- data_tree[-train_idx_tree, ]

train_idx_num <- createDataPartition(data_numeric$`Completed Admissions Process`, p = 0.8, list = FALSE)
...
dt_cm <- confusionMatrix(dt_pred, test_tree$`Completed Admissions Process`, positive = "Completed")
print(dt_cm)
...
nb_cm <- confusionMatrix(nb_pred, test_tree$`Completed Admissions Process`, positive = "Completed")
print(nb_cm)
...
svm_cm <- confusionMatrix(svm_pred, test_numeric$`Completed Admissions Process`, positive = "Completed")
print(svm_cm)
...
ann_cm <- confusionMatrix(ann_pred, test_numeric$`Completed Admissions Process`, positive = "Completed")
print(ann_cm)
